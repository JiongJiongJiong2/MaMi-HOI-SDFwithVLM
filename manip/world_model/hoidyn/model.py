...
class Dynamics(nn.Module):
    ...
    def compose_obj_motion_from_delta(
        self,
        obj_motion,
        pred_trans,
        pred_rot,
        ...
        """Convert predicted object deltas into normalized object states."""
        current = get_state_by_ts(
            obj_motion,
            anchor_ts,
            bt_merge=False,
        )
        if method == "fro":
            return current + torch.cat([pred_trans, pred_rot], dim=-1)
        if method == "theta":
            batch_size, num_steps, _ = current.shape
            pred_translation = current[:, :, :3] + pred_trans
            pred_rotation = torch.bmm(
                current[:, :, 3:].reshape(batch_size * num_steps, 3, 3),
                pred_rot.reshape(batch_size * num_steps, 3, 3),
            ).reshape(batch_size, num_steps, 9)
            return torch.cat([pred_translation, pred_rotation], dim=-1)
        raise NotImplementedError(method)

    def compute_dynamics_loss_for_generated_hoi(
