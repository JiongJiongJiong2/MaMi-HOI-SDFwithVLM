...
def get_world_human_geo(x, dataset, joints_only=True):
    try:
        ...
        from train.trainer_chois import run_smplx_model
...
def prepare_train_val_dataset_from_manifest(
    split_manifest,
    ...
    train_sequences = manifest.get("train_sequences")

    train_dataset, train_loader = prepare_dataset(
        train=True,
        batch_size=train_batch_size,
        window_size=window_size,
        data_root_folder=data_root_folder,
        allowed_sequence_names=train_sequences,
        num_workers=num_workers,
    )
    val_dataset, val_loader = prepare_dataset(
        train=False,
        batch_size=val_batch_size,
        window_size=window_size,
        data_root_folder=data_root_folder,
        allowed_sequence_names=validation_sequences,
        num_workers=num_workers,
    )
