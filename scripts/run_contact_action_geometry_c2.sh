...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/local_geometry_c2_20260915/$MODE}"
...
for seed in "${SEEDS[@]}"; do
  output_dir="$OUTPUT_ROOT/seed_${seed}/C2"
  mkdir -p "$output_dir"
  echo "running seed=$seed arm=C2"
  "$PYBIN" scripts/train_contact_action_world_model.py \
    ...
    --contrastive_objective predictive \
