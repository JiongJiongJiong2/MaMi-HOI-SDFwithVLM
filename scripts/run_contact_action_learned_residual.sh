...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/learned_residual_20260916/$MODE}"
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  EPOCHS=30
  read -r -a SEEDS <<< "${SEEDS_OVERRIDE:-21 22 23}"
  MAX_TRAIN=0
  MAX_VAL=0
  EVAL_BATCH=512
  BATCH_SIZE=512
...
for seed in "${SEEDS[@]}"; do
  output_dir="$OUTPUT_ROOT/seed_${seed}/B_learned_residual"
  mkdir -p "$output_dir"
  echo "running seed=$seed arm=B_learned_residual"
